...
export default function InteractiveMicrogridView() {
  ...
  return (
    <div className="space-y-6 max-w-[1680px] mx-auto pb-8 select-none">
      ...
      {statusMessage && (
        <div className="flex items-center justify-between rounded-2xl border border-[#DDE4DF] bg-[#E7F5EE] px-4 py-2.5 text-sm text-[#12372A] font-bold shadow-subtle">
      ...
      <div className="relative rounded-3xl border border-[#DDE4DF] bg-white p-5 sm:p-6 shadow-card overflow-hidden">
        
        {/* 3D Scene Header Bar with Clean Icon (NO floating stray dots) */}
        <div className="flex items-center justify-between pb-3 border-b border-[#DDE4DF]">
          ...
          <div className="flex items-center space-x-1.5">
            <button
              ...
              className="px-2.5 py-1 rounded-lg border border-[#DDE4DF] bg-[#F5F6F2] text-xs font-semibold text-[#142019] hover:bg-white transition hidden sm:inline-flex"
            >
              Top View
            </button>
            <button
        ...
        {/* 3D Viewport */}
        <div className="mt-4 h-[480px] sm:h-[540px] w-full relative rounded-2xl overflow-hidden bg-[#F5F6F2]">
          <MarketplaceScene3D
            ref={sceneRef}
          ...
          <div className="absolute top-3 left-3 pointer-events-none">
            <div className="flex flex-col gap-1.5 rounded-2xl border border-[#DDE4DF] bg-white/95 p-3 shadow-card backdrop-blur-md text-[11px] font-semibold text-[#142019]">
              <span className="text-[10px] uppercase font-bold text-[#7C8781]">Active Power Conduits</span>
              <div className="flex items-center gap-2">
                <FaIcon name="bolt" className="text-[#1C9A67] text-xs" />
                <span>P2P Peer Trade (Green)</span>
              </div>
              <div className="flex items-center gap-2">
                <FaIcon name="battery" className="text-[#E7A82D] text-xs" />
                <span>Battery Buffer (Amber)</span>
              </div>
              <div className="flex items-center gap-2">
                <FaIcon name="grid" className="text-[#3A78D1] text-xs" />
          ...
          {isDrawerOpen && selectedHouseholdData && (
            <div className="absolute top-3 right-3 w-80 rounded-2xl border border-[#DDE4DF] bg-white/98 p-5 shadow-modal backdrop-blur-xl animate-in fade-in slide-in-from-right duration-200 z-20">
              <div className="flex items-start justify-between pb-2.5 border-b border-[#DDE4DF]">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-[#7C8781]">
                    Node Inspector
                  </span>
                  <h4 className="text-base font-bold text-[#142019]">
                ...
                <button
                  ...
                  className="text-[#7C8781] hover:text-[#142019] text-xs font-bold p-1 rounded-lg hover:bg-[#F5F6F2]"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="mt-3 space-y-2.5 text-xs">
                <div className="flex justify-between py-1 border-b border-[#F6F7F4]">
                  <span className="text-[#5C6962]">Current State:</span>
                ...
                <div className="flex justify-between py-1 border-b border-[#F6F7F4]">
                  <span className="text-[#5C6962]">Solar Generation:</span>
                  <span className="font-mono font-bold text-[#E7A82D]">
                    {selectedHouseholdData.generation.toFixed(2)} kW
                  </span>
                </div>

                <div className="flex justify-between py-1 border-b border-[#F6F7F4]">
                  <span className="text-[#5C6962]">Active Consumption:</span>
                  <span className="font-mono font-bold text-[#3A78D1]">
                    {selectedHouseholdData.consumption.toFixed(2)} kW
                  </span>
                </div>

                <div className="flex justify-between py-1 border-b border-[#F6F7F4]">
                  <span className="text-[#5C6962]">Net Energy Balance:</span>
                  <span className={`font-mono font-bold ${selectedHouseholdData.netBalance >= 0 ? 'text-[#1C9A67]' : 'text-[#D95E5E]'}`}>
                ...
                {selectedHouseholdData.battery && (
                  <div className="flex justify-between py-1 border-b border-[#F6F7F4]">
                    <span className="text-[#5C6962]">Local Home Battery:</span>
                    <span className="font-mono font-bold text-[#D89A25]">
                ...
                <div className="pt-2">
                  <span className="text-[10.5px] font-bold text-[#7357C8] block mb-1">
                    AI Recommended Action:
                  </span>
                  <p className="text-[11.5px] text-[#5C6962] leading-relaxed">
        ...
        <div className="mt-3.5 pt-3 border-t border-[#DDE4DF] flex flex-wrap items-center justify-between gap-2 text-xs">
          <span className="text-[#5C6962] font-medium">Select Household Perspective:</span>
      ...
      <div className="rounded-3xl border border-[#DDE4DF] bg-white p-5 shadow-card">
        <div className="flex items-center justify-between pb-3 border-b border-[#DDE4DF] mb-3">
          <div className="flex items-center space-x-2.5">
            <div className="w-7 h-7 rounded-lg bg-[#E7F5EE] text-[#1C9A67] flex items-center justify-center text-xs">
              <FaIcon name="solar" />
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
          {activeFlows.map((flow) => (
            <div key={flow.id} className="flex items-center justify-between rounded-2xl border border-[#DDE4DF] bg-[#F5F6F2]/50 p-3.5 text-xs">
              <div className="flex items-center space-x-2.5">
                <FaIcon name="bolt" style={{ color: flow.color }} className="text-sm" />
                <span className="font-mono font-bold text-sm text-[#142019]">{flow.kw.toFixed(2)} kW</span>
                <Badge variant="default" size="xs">
                  {flow.type}
                </Badge>
              </div>
              <span className="font-mono font-bold text-[#1C9A67] text-xs">{flow.tariff}</span>
            </div>
          ))}
        </div>
      </div>
