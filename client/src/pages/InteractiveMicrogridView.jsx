...
export default function InteractiveMicrogridView() {
  ...
  return (
    <div className="space-y-4 max-w-[1680px] mx-auto pb-4">
      {/* Header bar */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-xs gap-3">
        <div className="flex items-center space-x-3.5">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-slate-900 text-teal-400 shadow-md">
            <FaIcon name="grid" className="text-lg" />
          </div>
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="text-base sm:text-lg font-bold text-slate-900">Energy Network & Digital Twin</h2>
              <Badge variant="surplus" size="xs">
                3D Live Simulation
              </Badge>
            </div>
            <p className="text-xs text-slate-500 font-normal">
              Real-time multi-node power flow, digital twin particle routing, and localized microgrid balancing.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <MetricCard
          title="Community Net"
          value={`${netCommunity >= 0 ? '+' : ''}${netCommunity.toFixed(1)}`}
          unit="kW"
          subtitle={netCommunity >= 0 ? 'Surplus Export/Store' : 'Deficit Demand'}
          iconName={netCommunity >= 0 ? "arrowUp" : "arrowDown"}
          variant={netCommunity >= 0 ? "surplus" : "deficit"}
        />

        <MetricCard
          title="Community Battery"
          value={`${battery.soc}%`}
          subtitle={`${battery.capacity} kWh Total`}
          iconName="battery"
          variant="battery"
        />
      </div>
      ...
      {demoStepMessage && (
        <div className="flex items-center justify-between rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-xs text-emerald-950 shadow-2xs">
          ...
          <button type="button" onClick={() => setDemoStepMessage('')} className="text-emerald-700 hover:text-emerald-950 font-bold text-xs p-0.5">
            ✕
          </button>
        </div>
      )}
